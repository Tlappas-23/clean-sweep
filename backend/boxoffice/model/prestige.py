...
GROUPS: dict[str, list[str]] = {
    "oscar pedigree": [
        "director_prior_oscar_noms", "director_prior_oscar_wins",
        "director_best_prior_noms", "cast_prior_oscar_noms",
        "cast_prior_oscar_wins", "cast_best_prior_noms",
        "writer_prior_oscar_noms", "writer_prior_oscar_wins",
        "writer_best_prior_noms", "has_oscar_winner_attached"],
...
def main() -> pd.DataFrame:
    ...
    rows = [{"group": "full v2 model", "n_features": len(every),
             "mae_log": full_mae, "within_2x": full_2x, "d_mae": 0.0, "d_2x": 0.0}]
    ...
    base = [c for c in every
            if not any(c in cols for cols in GROUPS.values())]
    base_mae, base_2x = score(frame, base)
    rows.append({"group": "all thirteen removed", "n_features": len(base),
                 "mae_log": base_mae, "within_2x": base_2x,
                 "d_mae": base_mae - full_mae, "d_2x": base_2x - full_2x})

    for name, cols in GROUPS.items():
        kept = [c for c in every if c not in cols]
        mae, two = score(frame, kept)
        rows.append({"group": f"- {name}", "n_features": len(kept),
