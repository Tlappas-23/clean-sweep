...
from boxoffice.model.train import feature_columns, run, summarise
from boxoffice.pipeline.fetch import BUDGET_FLOOR, DISTRIBUTORS
...
ax = axes[0]
steps = [("TMDB titles tagged theatrical, 2019", TMDB_THEATRICAL_2019, GREY),
...
ax.annotate("", xy=(1.72, mod_2x + 0.085), xytext=(1.28, bud_2x + 0.085),
            arrowprops=dict(arrowstyle="-|>", color=INK, lw=1.4,
                            connectionstyle="arc3,rad=-0.25"))
ax.text(1.5, mod_2x + 0.135, f"+{lift_pts:.1f} points", ha="center",
...
H.layout(fig, "evaluation",
  ...
  f"{N_FOLDS} rolling-origin folds, {FOLD_LO} to {FOLD_HI}, each scoring a year the model had "
  "never seen",
...
ax.set_title(f"Remove one group from all {N_FEATURES}: the real ranking", fontsize=10.5,
             color=INK, fontweight="bold", loc="left", pad=8)
H.frame(ax, grid="x"); ax.grid(axis="y", visible=False)
...
for yi, g in zip(y, order):
    ...
    ax.text(max(hi, d) + 0.35, yi, f"{d:+.1f} pts   p={r.p_mcnemar:.3f}" if r.p_mcnemar >= 0.001
            else f"{d:+.1f} pts   p<0.001", va="center", fontsize=9.2,
            fontweight="bold" if real else "normal", color=INK)
...
people_note = ("none of the five people-based groups clears the line"
               if people_real == 0 else
               f"{people_real} of the five people-based groups clear the line")
ax.text(0.985, 0.04, people_note, transform=ax.transAxes, ha="right",
