...
from house import ACCENT, GREY, LINE, MUTED  # noqa: E402  (sys.path is set two lines up, on purpose)
...
for arr, colour, label in [
    (p_prior, GREY, "budget-tier prior"),
...
H.layout(
    ...
    why_text=(
        "A probability is only worth quoting if it means what it says. The raw booster "
...
for arr, colour, label in [(p_prior, GREY, "budget-tier prior"), (p_model, ACCENT, "model")]:
    recall, precision = [], []
    for t in ts:
        flag = arr < t
...
H.layout(
    ...
    why_text=(
        "This is the curve a greenlight decision actually lives on. Budget tier is a strong "
